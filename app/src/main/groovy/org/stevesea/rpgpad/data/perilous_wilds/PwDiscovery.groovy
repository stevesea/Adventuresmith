...
class PwDiscovery extends AbstractGenerator {
    ...
    PwDetails pwDetails
    PwSteading pwSteading

    @Inject
    PwDiscovery(PwSteading pwSteading) {
        super(pwSteading.shuffler)
        this.pwCreature = pwSteading.pwCreature
        this.pwDetails = pwCreature.pwDetails
        this.pwSteading = pwSteading
    }
    ...
    static RangeMap arcane = new RangeMap()
            .with(1..2, 'residue')
            .with(3..5, 'blight')
...
""")
    ...
    static RangeMap terrain_change = new RangeMap()
    ...
    static RangeMap water_feature = new RangeMap()
    ...
    RangeMap landmark = new RangeMap()
            ...
            .with(11..12, "${ -> pwDetails.pickOddity()}")
    static RangeMap resource = new RangeMap()
...
<br/>${strong('Lair')}
<br/>${ -> pick(lair)}
<br/>${ss('Creature responsible:')} ${ -> pwCreature.generate()}
...
""")
            .with(5..7, """\
<br/>${strong('Terrain Change')}
<br/>${ -> pick(terrain_change)}\
""")
            .with(8..9, """\
<br/>${strong('Water Feature')}
...
""")
    ...
    static RangeMap infrastructure = new RangeMap()
            .with(1..4, 'track/path')
            .with(5..8, 'road')
            .with(9..10, 'bridge/ford')
            .with(11, 'bridge/ford')
            .with(12, 'mine/quarry')
            .with(11, 'aqueduct/canal/portal')
    static RangeMap dwelling = new RangeMap()
            .with(1..3, 'campsite')
            .with(4..6, 'hovel/hut')
            .with(7..8, 'farm')
            .with(9..10, 'inn/roadhouse')
            .with(11..12, 'tower/keep/estate')
    static RangeMap burial_religious = new RangeMap()
...
""")
    RangeMap discovery = new RangeMap()
            .with(1, """\
<br/>${strong('Unnatural Feature')}
<br/>&nbsp;&nbsp;${small('How does it affect its surroundings?')}
<br/>
<br/>${ -> pick(unnaturalFeature)}\
""")
            .with(2..4, """\
<br/>${strong('Natural Feature')}
<br/>&nbsp;&nbsp;${small('Describe how they notice it and what sets it apart.')}
<br/>
<br/>${ -> pick(naturalFeature)}\
""")
            .with(5..6, """\
<br/>${strong('Evidence')}
<br/>&nbsp;&nbsp;${small('Consider the implications and be ready for them to take the bait.')}
<br/>
<br/>${ -> pick(evidence)}\
""")
            .with(7..8, """\
<br/>${strong('Creature')}
<br/>&nbsp;&nbsp;${small('Not an immediate threat, but might become one.')}
<br/>
<br/>${ -> pwCreature.generate()}\
""")
            .with(9..12, """\
<br/>${strong('Structure')}
<br/>&nbsp;&nbsp;${small('Who built it? Is it connected to anything else they made nearby?')}
<br/>
<br/>${ -> pick(structure)}\
""")
