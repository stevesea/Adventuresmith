...
"""
    ...
    RangeMap occupation = new RangeMap()
            ...
            .with(2..6, "Commoner: ${ -> pick(commoner)}")
            .with(7..8, "Tradesperson: ${ -> pick(tradesperson)}")
            .with(9..10, "Merchant: ${ -> pick(merchant)}")
            .with(11, "Specialist: ${ -> pick(specialist)}")
            .with(12, "Official: ${ -> pick(official)}")

    RangeMap wilderness = new RangeMap()
            .with(1..2, "Criminal: ${ -> pick('1d8', criminal)}")
            .with(3..4, 'adventurer/explorer')
            .with(5..6, 'hunter/gatherer')
            .with(7..8, "Commoner: ${ -> pick(commoner)}")
    ...
    RangeMap rural = new RangeMap()
            ...
            .with(5..8, "Commoner: ${ -> pick(commoner)}")
            .with(9, "Tradesperson: ${ -> pick(tradesperson)}")
    ...
    RangeMap urban = new RangeMap()
            ...
            .with(4..7, "Commoner: ${ -> pick(commoner)}")
            .with(8, "Tradesperson: ${ -> pick(tradesperson)}")
            .with(9, "Merchant: ${ -> pick(merchant)}")
            .with(10, "Specialist: ${ -> pick(specialist)}")
            .with(11, 'militia/soldier/guard')
            .with(12, "Official: ${ -> pick(official)}")
