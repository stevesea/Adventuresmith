...
class SWNAnimal extends AbstractGenerator {
    Map<String, List<String>> templates_body_variations = [
            Insectile: [
                'Sucking mouthparts',
                'Chewing mouthparts',
                'Jewel-colored chitin',
                "2d4 pairs of limbs <small><em>(${ -> roll('1d4')})</em></small>",
                'Membranous wings',
                'Deposits eggs in live prey',
                'Has “silk” spinnerets',
                'Has a chrysalis life stage',
                'No mouth; adult form lives only to reproduce',
                "1d4 pairs of eyes or eyespots <small><em>(${ -> roll('1d4')})</em></small>",
                'Sluglike body',
            ...
            Reptilian: [
                'Sharp-edged scales',
                "1d4 pairs of eyes <small><em>(${ -> roll('1d4')})</em></small>",
                'Extremely long tail',
            ...
            Mammalian: [
                ...
                'Strictly nocturnal',
                "1d6+1 limbs, including any tail <small><em>(${ -> roll('1d6+1')})</em></small>",
            ...
            Avian: [
                'Sharp feathers',
                "1d3 pairs of wings <em>(${ -> roll('1d3')})</em>",
                'Long, sinuous neck',
            ...
            Exotic: [
                'Rocklike body',
                "1d4 pairs of eyestalks <small><em>(${ -> roll('1d4')})</em></small>",
                'Rolls on wheels',
                ...
                'Crystalline tissues',
                'Gas-sack body',
                "2d10 tentacles <small><em>(${ -> roll('2d10')})</em></small>",
...
<h4>Animal</h4>
{{template}}
<h5>Traits or Body Variations</h5>
&nbsp;&nbsp;{{trait_or_body}}
'''
    String generate() {
        Set<String> allExceptHybrid = new HashSet<>()
        allExceptHybrid.addAll(templates_body_variations.keySet())
        allExceptHybrid.remove('Hybrid')

        String template = pick(templates_body_variations.keySet())
        ...
        templates.each { it ->
            ...
            trait_or_body.addAll(pickN(templates_body_variations.get(it), roll('1d2+1')) as List<String>)
        }
